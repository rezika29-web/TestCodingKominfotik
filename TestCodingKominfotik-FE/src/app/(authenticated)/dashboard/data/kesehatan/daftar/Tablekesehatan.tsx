...
const TableKesehatan: React.FC = () => {
  ...
  const fetchData = async (page: number, size: number) => {
    try {
      const response = await api.get(`/kesehatan?page=${page}&pageSize=${size}`);
      ...
      const formattedData = roles.map((item: any, index: number) => ({
        ...
        rincian_gejala: item.rincian_gejala,
        diagnosa: item.diagnosa,
        test_yg_dilakukan: item.test_yg_dilakukan,
        status_diagnosa: item.status_diagnosa,
      }));
      setData(roles);
  ...
  const columns = [
    ...
    {
      title: "Rincian Gejala",
      dataIndex: "rincian_gejala",
      key: "rincian_gejala",
    },
    {
      title: "Diagnosa",
    ...
    {
      title: "Test Yang Dilakukan",
      dataIndex: "test_yg_dilakukan",
      key: "test_yg_dilakukan",
    },

    {
      title: "Status Diagnosa",
      dataIndex: "status_diagnosa",
      key: "status_diagnosa",
    },
    {
      title: "KETERANGAN",
  ...
  const handleEdit = (id: string) => {
    router.push(`/dashboard/data/kesehatan/edit/${id}`);
  ...
  const handleDelete = (key: string) => {
    ...
    Modal.confirm({
      title: "Konfirmasi",
      content: `Apakah Anda yakin ingin menghapus kesehatan "${itemToDelete.nama}"?`,
      okText: "Ya, Hapus",
      cancelText: "Batal",
      onOk: async () => {
        try {
          // Kirim permintaan DELETE ke API
          const response = await api.delete(`/kesehatan/${itemToDelete.id}`);

          if (response.status === 200) {
            message.success("kesehatan berhasil dihapus");
            ...
            message.error("Gagal menghapus kesehatan");
          }
        } catch (error) {
          console.error("Error deleting kesehatan:", error);
          message.error("Terjadi kesalahan saat menghapus kesehatan");
        }
      },
    });
  };
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-4xl font-bold">Data Kesehatan</h1>
        <div className="space-x-2">
          ...
          <CustomButton
            ...
            onClick={() => router.push("/dashboard/data/kesehatan/tambah")}
          >
            + Tambah Baru
          </CustomButton>
        </div>
